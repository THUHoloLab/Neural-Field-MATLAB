function [out_parser, depthF, w, s] = renderHier_occMC_old(dlo, ...
                                                      dld, ...
                                                      net_e,...
                                                      net_c,...
                                                      net_f, ...
...
x_emb = forward(net_e,dlarray(dlx_c,'CB'));
[dlsigma,dlrgb] = forward(net_c,x_emb,dlarray(dld_c,'CB'));
dlrgb = repmat(dlrgb,[3,1]);

dlsigma = dlsigma .* if_occ;
dlrgb = dlrgb .* if_occ;
...
x_emb = forward(net_e,pts_f);
[dlsigma_f,dlrgb_f]= forward(net_f,x_emb,viewdirs_ext_f);
dlrgb_f = repmat(dlrgb_f,[3,1]);

dlrgb_f = dlrgb_f .* if_occ;
dlsigma_f = dlsigma_f .* if_occ;
Esparse = 0;
...
out_parser = {rgbImageC,rgbImageF,Esparse,w};
