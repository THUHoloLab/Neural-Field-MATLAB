...
void Call_Fwd(
    ...
    const float3 *d_xys_batch = (const float3 * __restrict__ ) mxGPUGetDataReadOnly(xys_batch);
    const float4 *d_embedding = (const float4 * __restrict__ ) mxGPUGetDataReadOnly(embedding);
    ...
    CHECK_THROW((size_t) xys_sz.x == 3); // 3D positions
    ...
    float4 * d_output_embedding = (float4 *__restrict__) mxGPUGetData(output_embedding);
